...
def update_trie(hold_nodes: set, candidates: dict, trie: Node, key: str, score: float):
            ...
            if child.word_end:
                for word in child.word:
                    if word not in candidates:
                        candidates[word] = calculate_candidate_score(child)
...
def update_hold_nodes(hold_nodes: set, key: str, score: float):
    key = key.lower()
